...
function updateDocument(params) {
  return function (state) {
    ...
    var _expandReferences3 = (0, _languageCommon.expandReferences)(params)(state),
        database = _expandReferences3.database,
        collection = _expandReferences3.collection,
        filter = _expandReferences3.filter,
        changes = _expandReferences3.changes,
        options = _expandReferences3.options,
        callback = _expandReferences3.callback;

    var db = client.db(database);
    var mCollection = db.collection(collection);
    return new Promise(function (resolve, reject) {
      mCollection.updateMany(filter, {
        $set: changes
        ...
        if (err) {
          ...
          console.log("Updated a document matching ".concat(JSON.stringify(filter), " in the collection."));
          console.log(JSON.stringify(result, null, 2));
          var nextState = (0, _languageCommon.composeNextState)(state, result);
          if (callback) resolve(callback(nextState));
          resolve(nextState);
