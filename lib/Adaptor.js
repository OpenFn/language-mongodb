...
var request = require('sync-request'); // import request from 'request';
...
export function fetch2016(params) {
  var insertDocuments = function (db, jsonArray, callback) {
    ...
    var collection = db.collection('simce_2016_school_records'); // Insert some documents
  ...
  return state => {
    ...
    codes.forEach(function (code) {
      ...
      try {
        ...
        schools.push(jsonBody); // console.log(JSON.stringify(jsonBody, null, 2));
      } catch (e) {
        console.log("Failed to fetch RBD " + code);
      }
    }); // Connection URL

    var url = mongoConnectionUrl; // Use connect method to connect to the server
...
export function fetch2015(params) {
  var insertDocuments = function (db, jsonArray, callback) {
    // Get the documents collection
    var collection = db.collection('simce_2015_school_records'); // Insert some documents
  ...
  return state => {
    ...
    var url = mongoUrl; // Use connect method to connect to the server
...
;
export { field, fields, sourceValue, alterState, each, merge, dataPath, dataValue, lastReferenceValue } from 'language-common';
